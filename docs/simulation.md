...
- **Study** (`studies/libero_ee/`, this project's action-representation study registered as a
  ...
  (`replay_adapter.py`), the treatment specs (`configs/`), and the write-ups (`report.md`,
  `diary.md`, `README.md`) under `research/libero_ee/`.
...
```bash
uv run --package anvil-sim anvil-sim-bench status      # prints research/<study>/ledger/RESULTS.md (all topics)
```
...
| `record` | upsert `research/<study>/ledger/results.json` + regenerate `research/<study>/ledger/RESULTS.md` |
...
baseline (computed once per task, cached at `research/<study>/replay/baseline-task{N}`). If it falls
...
**Idempotency & control.** Each run writes `research/<study>/experiments/<name>/stage_status.json`; passed
...
A treatment = one YAML file. Most fields are derivable and can be omitted —
state only what makes the treatment distinct. Minimal (native baseline):
...
train: {action_type: ee_abs, policy_type: act}
eval:  {action_type: zerocal_goal_abs, n_episodes: 50}
```

**What's derived when omitted** (explicit values always override):
- `train.trainer`: `"anvil-trainer"` if `train.action_type` is set, else `"lerobot-train"`.
- `train.steps`: 50000 for `policy_type: act`, 30000 for `diffusion`.
- `train.batch_size`: 16.
- `env_suite` / `env_task_id`: filled from `task_index` by the study (`Study.fill_defaults`).
- `eval.control_mode`: filled from `eval.action_type` — the deliver↔control_mode mapping
  Experiment 7 got wrong is applied automatically; illegal explicit pairings are still
  rejected at load time.
- `gates.gt_replay_margin`: 15.0.
...
forbidden for lerobot-train), `policy_type` (`act`|`diffusion`), `steps`, `batch_size`,
...
`load_spec` **rejects unknown keys** (top-level and per-section — typos fail loud), fills the
derived fields above via `Study.fill_defaults`, then runs `validate()` (enum checks,
trainer↔action_type rules, and the study's legality rules such as dataset-group membership and
deliver↔control_mode pairing).
...
Everything for one study/topic lives under `research/<study>/` (here `research/libero_ee/`), so the
write-ups sit next to the raw experiments that back them — layout: see
[`research/README.md`](../research/README.md).

The one harness-specific detail not covered there: `ledger/RESULTS.md`'s columns are name, task,
dataset, policy, eval type, mode, n, replay GT/base, pc_success, recorded. `anvil-sim-bench status`
prints the ledger for every topic under `research/` (or one topic via `--study <topic>`). Model
checkpoints live separately under `model_zoo/research/<study>/<name>/`.
...
| `anvil-eval-libero --action-type=<t> [--trace-dir=<d>] --policy.path … --env.type=libero --env.task … --env.task_ids='[N]' --env.control_mode … --eval.n_episodes … --output_dir …` | ad-hoc closed-loop eval of a checkpoint (`--action-type=native_rot6d` included) |
