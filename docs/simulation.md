...
- **Study** (`studies/libero_ee/`, this project's action-representation study registered as a
  ...
  (`replay_adapter.py`), the treatment specs (`configs/`), and the write-ups (`report.md`, `diary.md`) now under `research/libero_ee/`.
...
write-ups sit next to the raw experiments that back them:

- **Write-ups (git-tracked):** `report.md` (conclusions), `diary.md` (chronological log incl. the
  wrong turns), and `README.md` — a guide that maps every result file to the claim it supports.
- **Ledger (git-ignored, machine-generated by `record`, never hand-edited):** `ledger/RESULTS.md`
  (table: name, task, dataset, policy, eval type, mode, n, replay GT/base, pc_success, recorded)
  and `ledger/results.json` (same rows as JSON, upserted by name).
- **Per-experiment raw results (git-ignored):** `experiments/<name>/` — `stage_status.json`,
  `<stage>.log`, `gt-replay/`, `eval/eval_info.json` (+ videos), `smoke_eval/` (+ `smoke-*.log`).
- **Analysis (git-ignored):** `analysis/` — generated figures/JSON.
- **GT-replay traces (git-ignored):** `replay/` — cached per-task baselines + debug traces.

`anvil-sim-bench status` prints the ledger for every topic under `research/` (or one topic via
`--study <topic>`). Model checkpoints live separately under `model_zoo/research/<study>/<name>/`.
