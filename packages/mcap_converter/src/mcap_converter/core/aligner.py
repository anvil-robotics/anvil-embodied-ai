...
from ..config.schema import DataConfig, DEFAULT_DATA_CONFIG, FeatureMapping
...
class TimeAligner:
    ...
    def align_sensors(
        ...
        print(f"Using {main_cam_name} as main time axis, total {len(main_cam_info['timestamp'])} frames")
        ...
        for robot in robot_prefixes:
            for role in ['observation', 'action']:
                ...
                if key in extracted_data:
                    joint_data[(robot, role)] = self._prepare_joint_data(
                        extracted_data[key],
                        role
                    )
