...
def inspect_message_structure(
    mcap_path: str, topic: str = None, max_samples: int = 5
) -> Dict[str, Any]:
    """
...
def format_output(topic_info: Dict, structure_info: Dict, output_format: str = "text"):
    ...
    if structure_info:
        ...
        for topic_name, struct_info in sorted(structure_info.items()):
            ...
            output_lines.append(
                f"  Time range: {struct_info['first_timestamp']:.6f} - {struct_info['last_timestamp']:.6f} s"
            )
            ...
            for field_name, field_info in sorted(struct_info["fields"].items()):
                ...
                if "length" in field_info:
                    output_lines.append(
                        f"    {field_name:<60} {field_type} (length: {field_info['length']})"
                    )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze MCAP file structure, topics, and message types",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""\
examples:
  mcap-inspect path/to/file.mcap
  mcap-inspect path/to/file.mcap --topic /leader1/joint_states
  mcap-inspect path/to/file.mcap --format json --output report.json
...
""",
    ...
    parser.add_argument(
        ...
        help="only analyze specified topic (default: all topics)",
    )
    parser.add_argument(
        "--max-samples", type=int, default=5,
        help="max message samples to analyze per topic (default: 5)",
    )
    parser.add_argument(
    ...
    try:
        ...
        structure_info = inspect_message_structure(
            str(mcap_path), topic=args.topic, max_samples=args.max_samples
        )
