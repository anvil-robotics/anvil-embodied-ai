"""Derive the LIBERO-EE study's dataset groups from a single `lerobot/libero` task.

``native`` is written here too, as a plain LOCAL copy of the source's own
8-dim/7-dim schema (no Anvil transform at all — see :func:`_make_native_dataset`).
...
a local subset dataset sidesteps it entirely: no Hub-side episode filtering
happens at load time, so metadata and the loaded rows are the same size and
global == relative.

``native_rot6d`` (see :func:`native_action_to_rot6d`) keeps native's
observation AND action values completely unchanged (same 8-dim state, same
controller-scale numbers, no calibration at all) and only re-encodes the
action's rotation component as rot6d via a lossless, invertible
axis-angle-shaped embedding — isolates rotation ENCODING with no physical-unit
conversion, no approximation error. It trains via plain ``lerobot-train``
(same path as ``native``, no ``anvil-trainer`` at all).

``native_hand`` (see :func:`native_action_to_hand`) rotates native's own
world-frame command into the EE body (hand) frame, isolating the FRAME factor
(world vs hand) with the native representation — same 8-dim observation and
plain ``lerobot-train`` path as ``native``/``native_rot6d``.

``goalabs`` and the native-family control-factor conditions (``native_abs``,
``native_n0``) are built from a FORMAL goal composition, ``G[t] = state[t] +
native_delta[t]`` (UNSCALED — see :func:`native_delta_to_goal`'s docstring for
why an earlier version that scaled this by robosuite's ``output_max`` caused
catastrophic closed-loop failure). ``goalabs`` stores it as the 10-dim rot6d
action (with Anvil's quat observation, trained via ``anvil-trainer``);
``native_abs``/``native_n0`` store the axis-angle equivalent alongside
NATIVE's raw 8-dim observation (trained via plain ``lerobot-train``) —
``native_abs`` is the absolute goal, ``native_n0`` is that goal relativized
per-frame against its own observed EE pose (world frame). All are eval'd by
RECOVERING a native-delta-shaped quantity relative to the REAL current state
at eval time (see ``anvil_sim.libero_processor.recovered_delta_native_action``)
and feeding it to ``env.control_mode="relative"`` — letting robosuite's own
``scale_action`` apply the true physical scale, so nothing here needs to know
or guess what that scale actually is.
"""
...
def goal_state_to_axis_angle_action(goal_state8: np.ndarray) -> np.ndarray:
    ...
    to rot6d losslessly at eval. Shared encoder for the ``native_abs`` action
    column, which differs from the rot6d ``goalabs`` family ONLY in this
    rotation encoding. Gripper is the goal's native +/-1 command
    (``native_delta_to_goal`` passthrough), carried through unchanged.
    """
    pos = goal_state8[:3]
...
def convert_episode_goal_aa_actions(goal_states: np.ndarray) -> np.ndarray:
    ...
    the ``native_abs`` group's action column (see
    :func:`goal_state_to_axis_angle_action`). Axis-angle counterpart of
    :func:`convert_episode_goal_abs_actions`.
    """
...
def native_action_to_rot6d(native_action7: np.ndarray) -> np.ndarray:
    ...
    re-encoding its rotation component as rot6d — isolates whether rot6d
    encoding *itself* affects closed-loop success vs native's own axis-angle
    encoding, with no other confounds.

    This does NOT convert to physical units or apply any calibration scale.
    ``native_action7[3:6]`` is the controller's own command-scale numbers,
    NOT real radians. Here we don't care what they physically mean: we just treat them
...
def _make_native_dataset(output_dir: Path, repo_id: str) -> LeRobotDataset:
    ...
    # "agentview"/"wrist" rename used for goalabs -- native has no custom
...
def convert(
    ...
    ``native`` to consume directly via
    ...
    Args:
        only: If given, restrict dataset CREATION to this subset of
            :data:`ALL_DATASET_GROUPS` (e.g. ``{"native_n0"}`` to add one more
            group to an output_root that already has others written, without
    ...
    for ep_num, ep_global_idx in enumerate(episodes):
        ...
        # goalabs / native_abs / native_n0 — the formal goal target family, see
        ...
        # native_abs: absolute goal in axis-angle, stored alongside NATIVE
        # 8-dim obs. native_n0: that goal relativized per-frame against its
        # own observed EE pose (n-0, world frame) — see
        ...
        for t, item in enumerate(ep_items):
            ...
            # reconstructed goal G[t] (see native_delta_to_goal).
            frame_goalabs = {
...
def main() -> None:
    ...
    parser.add_argument(
        ...
        help=(
            ...
            f"({sorted(ALL_DATASET_GROUPS)}), e.g. --only=native_n0 to add one "
            "more group without touching an output_root that already has the "
            "others. Default: write all groups."
        ),
    )
