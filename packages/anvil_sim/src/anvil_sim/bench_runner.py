...
4.  ``gt-replay``         HARD GATE: dataset ground truth must succeed
                          ...
                          replay baseline on the same episodes, at the SAME
                          n_episodes as the treatment (the baseline cache is
                          keyed on task_index AND n_episodes — see
...
def _replay_baseline(spec: BenchSpec) -> dict:
    """Study baseline GT-replay for this task, computed once per
    (task_index, n_episodes) and cached.

    # INVARIANT: the cache key MUST include n_episodes, not just task_index.
    Bug found 2026-07-13: the old key was `baseline-task{task_index}` alone,
    so the first-written baseline (e.g. n=10) was silently reused to gate
    every later condition on the same task run at a different n_episodes
    (e.g. n=49/50) — every `/base` column produced under that key is stale.
    See research/libero_ee/stage1-closeout.md. The cached payload already
    records its own n_episodes (eval_replay.py's replay() summary), so this
    key just makes that fact visible on disk instead of silently ignored.
    """
    baseline_dir = (
        topic_root(spec.study_name) / "replay"
...
def stage_record(spec: BenchSpec) -> dict:
    ...
    entry = {
        ...
        "recorded_at": _now(),
        # Purely informational per-condition flag (e.g. "⚠ provisional", "⛔
        # invalid") — the study classifies, the harness just surfaces it. See
        # Study.condition_status / research/libero_ee/stage1-closeout.md.
        "status": spec.study.condition_status(spec) if spec.study.condition_status else "—",
    }
...
def _write_results_md(results: list[dict], md_path: Path) -> None:
    lines = [
        ...
        "",
        "_`replay (GT/base)` baselines cached before 2026-07-14 may be gated against a"
        " stale n=10 baseline (bug fixed that day — the cache key now includes"
        " n_episodes, see `_replay_baseline`); re-run `gt-replay --force-stage` on an"
        " older row if its `status` looks off. See `research/libero_ee/stage1-closeout.md`._",
        "",
        "| name | task | dataset | policy | eval type | mode | n | replay (GT/base) | pc_success | status | recorded |",
        "|---|---|---|---|---|---|---|---|---|---|---|",
    ]
    for r in results:
        ...
        lines.append(
            ...
            f"| **{r['pc_success']}** | {r.get('status', '—')} | {r['recorded_at']} |"
        )
    md_path.write_text("\n".join(lines) + "\n")
...
def _clear_stage_cache(stage: str, spec: BenchSpec) -> None:
    ...
    idempotent path — ``stage_eval`` reuses ``eval_info.json`` if present, and
    ``stage_gt_replay`` reuses the cached baseline via ``_replay_baseline``.
    Those optimizations silently defeat ``--force-stage eval``/``gt-replay``
    (the stage function reruns but returns the stale number), so when a stage
    is explicitly forced we remove its cache first."""
