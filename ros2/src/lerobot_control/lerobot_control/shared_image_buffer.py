...
class SharedImageBuffer:
    ...
    def read(self, camera_name: str) -> tuple[np.ndarray, float, int]:
        ...
        timestamp = struct.unpack(
            "d", bytes(buf[self.image_size : self.image_size + self.TIMESTAMP_SIZE])
        )[0]
...
class SharedJointStateBuffer:
    ...
    def __init__(
        self, num_joints: int, create: bool = True, buffer_name: str = "lerobot_joint_state"
    ):
        ...
        if create:
            ...
            self._shm = shared_memory.SharedMemory(
                name=buffer_name, create=True, size=self.buffer_size
            )
    ...
    def write(self, positions: np.ndarray, timestamp: float):
        ...
        # Write timestamp
        buf[self.positions_size : self.positions_size + self.TIMESTAMP_SIZE] = struct.pack(
            "d", timestamp
        )
    ...
    def read(self) -> tuple[np.ndarray, float, int]:
        ...
        timestamp = struct.unpack(
            "d", bytes(buf[self.positions_size : self.positions_size + self.TIMESTAMP_SIZE])
        )[0]

        return positions, timestamp, counter

    def read_if_new(self) -> tuple[np.ndarray, float] | None:
