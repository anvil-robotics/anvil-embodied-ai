"""
Multi-Process Strategy (mode: mp)

Uses separate worker processes for image acquisition, providing better
process isolation and crash resilience. This is the default mode.
...
- Worker crashes don't affect the main inference process
- Cleaner separation of concerns
- Better scalability for many cameras

Note: Both strategies can achieve ~30 Hz since OpenCV releases the GIL
during JPEG decompression. The multi-process architecture is recommended
for deployments due to better isolation.
"""
...
class MultiProcessStrategy:
    ...
    def __init__(self):
        ...
        self._image_buffer: Optional[SharedImageBuffer] = None
        ...
        self._joint_positions: Optional[Dict[str, float]] = None
    ...
    def _setup_shared_memory(self) -> None:
        ...
        self._node.get_logger().info(
            f"Created shared memory for {len(self._camera_names)} cameras"
        )
    ...
    def _start_workers(self) -> None:
        ...
        for topic, camera_name in self._camera_mapping.items():
            ...
            self._node.get_logger().info(
                f"Started worker: {topic} -> {camera_name} (PID: {p.pid})"
            )
