...
class ActionLimiter:
    ...
    def __init__(
        self,
        max_delta: float = 0.1,
        model_joint_order: Optional[List[str]] = None,
        controller_joint_order: Optional[List[str]] = None,
    ...
    def _build_reorder_indices(self) -> List[int]:
        ...
        if len(self.model_joint_order) != len(self.controller_joint_order):
            self._log("warn", f"Joint order lengths differ: model={len(self.model_joint_order)}, controller={len(self.controller_joint_order)}")
            return []
