...
class ModelLoader:
    ...
    def chunk_size(self) -> Optional[int]:
        ...
        if self._model and hasattr(self._model, 'config') and hasattr(self._model.config, 'chunk_size'):
            return self._model.config.chunk_size
        return None

    @property
    def n_action_steps(self) -> Optional[int]:
        """Get model n_action_steps from config."""
        if self._model and hasattr(self._model, 'config') and hasattr(self._model.config, 'n_action_steps'):
    ...
    def _create_temporal_ensembler(self, model, coeff: float) -> None:
        ...
        try:
            ...
            self._log("info", f"Created temporal ensembler (coeff={coeff}, chunk_size={chunk_size})")
        except ImportError as e:
    ...
    def load_with_processors(self) -> Tuple[Any, Any, Any]:
        ...
        try:
            ...
            if pre_config.exists():
                ...
                if hasattr(pre_processor, 'to') and callable(getattr(pre_processor, 'to')):
                    try:
            ...
            if post_config.exists():
                ...
                # Move to device if the pipeline supports it
                if hasattr(post_processor, 'to') and callable(getattr(post_processor, 'to')):
                    try:
