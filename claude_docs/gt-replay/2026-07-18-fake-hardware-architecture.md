...
| `home_atol_pos_m` / `home_atol_rot_deg` | replayer ROS params | `0.05` / `10.0` | Homing arrival tolerance — a coarser "did we arrive" check, NOT `GtReplayVerifierNode`'s trajectory-match tolerance. Well above `anvil_eval`'s real-hardware pass/fail threshold (`0.02m`/`5.0deg`, `metrics.py`) — live real-hardware testing showed the controller plateauing anywhere from ~0.022m/5.9deg up to ~0.039m/9.1deg across different attempts, so this sets margin above the worst observed value rather than sitting right at the edge of it |
